...
rm -f temp1
rm -f tablenames_hive_location
rm -f script_hive_location
rm -f script_hiveop_location
...
var_tablenames_location="/tablenames_hive_location"
...
echo " "
hive -f tablenames_hive_temp >>tablenames_hive_1
cat tablenames_hive_1 |head -2 >>tablenames_hive_location
if [ "$?" -eq 0 ];
	then
	echo " "
...
#-----------------------------------------------------------------------------------
#fetching hdfs location and the details from hdfs
...
#hdfs_details : size and location of all tables from the hdfs location
#-----------------------------------------------------------------------------------
echo "fetching hdfs location and the details from hdfs"
echo "set hive.cli.errors.ignore=true;">>script_hive_location
while read line
	do
	echo $var_describe_formatted$database$var_dot$line$var_semicolon>>script_hive_location
	done <$var_current_directory$var_tablenames_location
hive -f script_hive_location >>script_hiveop_location

var_hdfs_location=$(cat script_hiveop_location| grep "Location" |head -1|awk '$1=$1'|sed -e 's/Location: //g'|sed 's/[^/]*$//')
echo " "
...
awk 'FNR==NR{A[$1]=$0;next} ($1 in A){print A[$1]}' tablenames_hdfs tablenames_hive_1 >>tablenames_hive
...
echo " "
echo "set hive.cli.errors.ignore=true;">>script_hive
while read line
	do
...
var_no_of_tables=$(cat output_hive.csv |wc -l)
echo " "
echo "Top $var_no_of_tables tables in term of size are at $var_current_directory/output_hive.csv"
echo "Top $var_no_of_directories directories in terms of size are at $var_current_directory/output_hdfs.csv"
echo " "
#-----------------------------------------------------------------------------------
#Sending the Outputs to the mail_id
#-----------------------------------------------------------------------------------
echo "Please find the attachments for the top $top high volume tables in output_hive.csv and high volume directories (if any) in output_hdfs.csv for the database $database" |mail -s "HDFS space consumption" -a 'output_hive.csv' -a 'output_hdfs.csv' $mail_id
#-----------------------------------------------------------------------------------
...
#rm -f temp1
#rm -f diff_hive_hdfs
#rm -f rm -f tablenames_hive_location
#rm -f script_hive_location
#rm -f script_hiveop_location
